...
// Get recent bookings
$stmt = $pdo->prepare("SELECT b.*, f.name as field_name FROM bookings b JOIN fields f ON b.field_id = f.id WHERE b.user_id = ? ORDER BY b.created_at DESC LIMIT 3");
...
<head>
    ...
    <title>Dashboard - Mini Soccer Booking</title>
...
<body class="bg-slate-50">
    ...
    <nav class="bg-white shadow-sm border-b border-slate-200">
                ...
                <div class="flex items-center">
                    ...
                    <div class="hidden md:block ml-10">
                        <div class="flex space-x-4">
                            <a href="dashboard.php" class="bg-emerald-50 text-emerald-700 px-3 py-2 rounded-md font-medium">Dashboard</a>
        ...
        <div class="hidden md:hidden border-t border-slate-200 bg-white absolute w-full z-50 shadow-lg" id="mobile-menu">
            <div class="px-2 pt-2 pb-3 space-y-1">
                <a href="dashboard.php" class="bg-emerald-50 text-emerald-700 block px-3 py-2 rounded-md text-base font-medium">Dashboard</a>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8">
            ...
            <p class="text-slate-500 mt-1">Selamat datang kembali di sistem pemesanan lapangan.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <!-- Quick Action Card -->
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col items-center justify-center text-center hover:shadow-md transition">
                <div class="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center text-2xl mb-4">📅</div>
                <h3 class="text-lg font-bold text-slate-800 mb-2">Pesan Jadwal Baru</h3>
                <p class="text-sm text-slate-500 mb-4">Cari lapangan kosong dan booking sekarang juga.</p>
                <a href="booking.php" class="w-full bg-emerald-600 text-white font-medium py-2 rounded-lg hover:bg-emerald-700 transition">Booking Lapangan</a>
            </div>
            
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col items-center justify-center text-center hover:shadow-md transition">
                <div class="w-16 h-16 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center text-2xl mb-4">📄</div>
                <h3 class="text-lg font-bold text-slate-800 mb-2">Cek Riwayat</h3>
                <p class="text-sm text-slate-500 mb-4">Lihat status pesanan dan tagihan yang belum dibayar.</p>
                <a href="riwayat.php" class="w-full bg-indigo-600 text-white font-medium py-2 rounded-lg hover:bg-indigo-700 transition">Lihat Riwayat</a>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                <h3 class="text-lg font-bold text-slate-800">Pesanan Terakhir Anda</h3>
                <a href="riwayat.php" class="text-sm text-emerald-600 font-medium hover:text-emerald-800">Lihat Semua &rarr;</a>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Lapangan</th>
